...
int main(){
	...
	initGame(&screen, button, &menu, &menutop, &menuExit, &MenuMusic, &SoundButtonHover, &SoundButtonClick, &menuSingleplayer);
	cinematiqueLJ(screen);
	Mix_PlayMusic(MenuMusic, -1);
	while(!done){
		choix = mainMenu(screen, menu, menutop, button, SoundButtonHover, SoundButtonClick, &poskeys);
		switch(choix){
			case 1:
